    ...
    reducers: {
        next(state, action) {
            ...
            if(state.filterOn === true) {
                ...
                let cardCount = state.cards.length;
                cardCount--;
                // binarySearch(arr, l, r, i)
                let cardIndex = binarySearch(state.cards,0,cardCount,nextSetCardId);
                state.currentCard = state.cards[nextSetCardId];
            }
            else {
