...
// app.use(session( { secret:'BigDogBisesi'} ));
...
router.post('/register', async (req, res) => {
  ...
  if( usernameExists.length === 0 ){
    ...
    */
    ...
    const token = null;//sign(payload, process.env.PRIVATE_KEY, options);
...
router.post('/login', async (req, res) => {
  ...
  if(userQueryResponse.length === 1){
    ...
    if(compareSync(pw,user.password)){
      ...
      const options = { algorithm: 'RS512' };
      const token = null;//sign(payload, process.env.PRIVATE_KEY, options);
      const body = {'loggedInUser':user,'cards':cardsResponse,'avatar':avatar,'sessions':sessions,'cardSetCards':cardSetCards,'cardSetData':cardSetData,'token':token};
      res.status(200).type('application/json').send(body);
    } else {
