...
export default function PointAnimation() {
  ...
  const nailed = useSelector(state => state.testSlice.session.nailed);
  const streak = useSelector(state => state.testSlice.session.streakClass);
  const [ localNailed, setLocalNailed ] = useState(0);
  ...
  if(nailed !== 0 && bingoRef !== null && starRef !== null) {
    // console.log(`local val: ${localNailed} in store: ${nailed}`);
    if( nailed > localNailed ) {
      // Only play animation if nailed in store has changed, that is only if the nailed button was pushed.
      // Since they both start at 0, as soon as nailed in store goes up play animation and update the local nailed.
      setLocalNailed(nailed);
      switch(streak) {
