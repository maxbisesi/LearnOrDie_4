...
router.post('/createSet', async (req,res) => {
  ...
  const _desc = description.replace(`'`,`\'`);
  ...
  for(let j = 0; j<cardsInSet.length; j++) {
     cscardRes = await dataDAO.runQuery(queries.insertCardSetCard,[cardsInSet[j].card_id,cardsetId]);
  ...
  res.status(200).type('application/json').send({'set_id':cardsetId, 'setname':setname, 'description':description, 'cards':cardsInSet});
});
