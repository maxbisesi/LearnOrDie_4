...
router.post('/register', async (req, res) => {
  ...
  if( usernameExists.length === 0){
    ...
    let registeredUser = await dataDAO.query(queries.createUser,[un,pw,em]);
    ...
    // - - - - - - - - - - - - - - - -
    let userWeapon = await dataDAO.query(queries.createWeapon,[wp,damage,defense]);
    const weaponId = userWeapons.insertId;
    // Insert Avatar using previous two ids...
    let insertedAvatar = await dataDAO.query(queries.createAvatar, [sty,bs,userid,wepId,av,1]);
