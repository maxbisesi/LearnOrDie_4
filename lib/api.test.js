...
describe('Test the api calls', () => { 
    ...
    afterAll( async(done) => {
        ...
        const delTestCard = db.query('DELETE FROM FlashCards WHERE card_id=?',[testCardId]);
        expect(delTestCard.affectedRows).toBe(1);

        const delTestSession = db.query('DELETE FROM UserSessions WHERE session_id=?',[testSessionId]);
        expect(delTestSession.affectedRows).toBe(1);

        const delTestUser = db.query('DDELETE FROM FlashUsers WHERE user_id=?',[testUserId]);
