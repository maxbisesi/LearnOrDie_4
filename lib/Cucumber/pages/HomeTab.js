...
export default class HomeTab extends TemplatePage {
    ...
    async removeCategory(collection,category) {
        ...
        row.findElement(By.className(`home-collection-removecategory`)).click();
        return true;
    }
    ...
    async selectCheckBox(checkboxName) { 
        ...
        const catCheckbox = await this.driver.wait(until.elementLocated(By.xpath(`//input[@name="${checkboxName}"]`), 2000));
        logger(` found checkbox for ${checkboxName}`);
        await catCheckbox.click();
        return true;
    }
