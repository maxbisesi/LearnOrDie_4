...
import { login, rankup } from './userSlice';
import { setCards } from '../Cards/cardSlice'; 
...
export default function Login(props){
  ...
  async function handleSubmit(event) {
    ...
    const cards = loginResponse.data.cards;
    let {user_id, userName, firstName, lastName, email, totalPoints, cardCount, rank, firstTimeUser } = loginResponse.data.loggedInUser;
    const badges = 0;

    if (loginResponse.status === 200) {
      // Get the cards and catgories for this user
      dispatch(setCards(loginResponse.data.cards));
      //If they were a first time user but they just logged in again, set them firstTimeUser: false
      if(firstTimeUser === true) { firstTimeUser = false; }
      // After setting the cards set the user in the store
