...
class DataApi {
  ...
  async addCard({
    ...
    const db = this.getDatabase();
    try {
      const card = db.query(newCard, [
        answer,
        card,
        category,
        times_wrongtimes_right,
        fk_user_id
      ]);
  ...
  async updateCard({
    ...
    const card = `Update flashcard Set card=?, answer=?, times_wrong=?, times_right=?, category=? Where cardid=?`;

    const db = this.getDatabase();
    try {
      const card = db.query(card, [
        card,
        answer,
        times_wrong,
        times_right,
  ...
  describeError(err) {
    ...
    console.log(
      chalk.yellowBright.bold.bgWhite(`    Failed Query: ${err.sql}\n`)
    );
  }
}
