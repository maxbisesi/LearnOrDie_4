...
router.post('/register', async (req, res) => {
  ...
  // Check username flow
  if(em === undefined && av === undefined) { 
      let exists = await dataApi.checkCredentials(un, pw);
...
router.post('/login', async (req, res) => {
  ...
  const user = dataDAO.query(queries.getUser,[un]);
  ...
  if(user.length > 0){
    ...
    if(user.password === pw) {
      ...
      const cardsets = dataDAO.query(queries.getCardSetCards,[user.user_id]);
      const sessions = dataDAO.query(queries.getUserSessions,[user.user_id]);

      const body = {'loggedInUser':user,'cards':cardsResponse,'avatar':avatar,'sessions':sessions,'cardSetMap':cardsets};
