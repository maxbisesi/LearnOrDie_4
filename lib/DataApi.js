...
class DataApi {
    ...
    checkCredentials(un, pw) {
        ...
        return { name: 'Big Dog Bisesi', username: 'max', password: 'pass' };
    }
    ...
    getCardsForUser(userid) {
        const query =`
            ...
            owner_id, fk_user_id 
            from flashcard Where fk_user_id=${userid} LIMIT 100`;

        this.#pool.getConnection((err, connection) => { 
            if (err) {
                console.log(chalk.bgRed.black(`mysql error in getCardForUser() connection`));
                this.describeError(err);
                throw new Error('mysql error in getCardForUser() connection')
                return;
            }

            connection.query(query, (err, results, fields) => { 
                //console.log(chalk.bgGreen(`Results Length: ${results.length}`));
                //console.log(chalk.bgYellow(`Results: ${JSON.stringify(results)}`));
                console.log(chalk.bgBlack.red(Object.keys(results[0])));
            });
