...
export default function Chum() {
  ...
  async function submitCard(key) {
    if(state.card === '' || state.answer === '' || state.category === ''){
      return;
    }
    ...
    let insertCardStatus;
    try {
      insertCardStatus = await dispatch(insertCard(payload));
    } catch(e) {
      logger(`Chum: Error inserting card: ${e}`);
      insertCardStatus = 'Insert Card ERROR';
    }    
    
    // reset form 
    console.log(`dispatch return: ${insertCardStatus}`);
    if(insertCardStatus === 'Insert Card Successful') {
      cardTl.play();
      localDispatch({'type':'clearQandA'});
      // Check for prizes
    } else {
      alert('Whoops, there was an error saving your card, save the text locally then refresh and restart.');
    }
