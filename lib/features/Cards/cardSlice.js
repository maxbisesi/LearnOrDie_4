    ...
    reducers: {
        ...
        unStudySet(state,action) {
            ...
            if(index !== -1) {
                ...
                const unstudiedSet = state.cardSets.find( set => set.set_id == removedSetId );
                state.studySetCardIds = state.studySetCardIds.filter( cardid => !unstudiedSet.cards.includes(cardid));
                if(state.studySetCardIds.length === 0) {
                    ...
                    for(let i = 0; i < state.studySetIds.length; i++) {
                        // Get card IDs from set
                        const cardset = state.cardSets.find( set => set.set_id == state.studySetIds[i].set_id );
                        cardset.cards.forEach( (c) => {
                            state.studySetCardIds.push(c);
                        });
                    }
