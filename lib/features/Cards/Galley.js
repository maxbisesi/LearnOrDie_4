...
export default function Galley(props) {
    ...
    if( showSearched === false) {
        ...
        let len = cards.length;
            for(let x = 0; x < len; x++) {
                let c = cards[x];
                if(c.card.includes(searchVal) || c.answer.includes(searchVal) || c.category.includes(searchVal)) {
                    console.log(`searched for and found card_id: ${c.card_id}`);
                    let qClicked = false;
                    let qDeleted = false;
                    if(clicked.includes(c.card_id)) { qClicked = true; }
                    if(deleted.includes(c.card_id)) { qDeleted = true; }
      ...
      <div className="galley">
        ...
        <div>
          <ul className="galley-question_table">
              { showSearched ? searchCards : pageCards }
          </ul>
        </div>
        ...
        <div className="galley-searchBar">
            <input  onChange={(e) => setSearchVal(e.target.value)} id="galleysearchbar" type="search" size="50"/>
            <button className="galley-searchbutton" onClick={search} >Search</button>
