...
import { randomData } from './Cucumber/randomData';
...
class Utils {
  ...
  checkRankUp(currentRank, currentPoints, badges ) {
   ...
   binarySearchForCardId(A,I) {
    ...
    getRandomWords() {
      let inputlength = randomData.words.length;
      let sentanceLength = (Math.random()*20);
      let sentance = '';
      for(let i = 0; i < sentanceLength; i++) {
        sentance += randomData.words[Math.floor(Math.random()*inputlength)];
      }
      return sentance;
    }
