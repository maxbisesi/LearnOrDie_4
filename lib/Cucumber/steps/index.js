...
When('The client registers a new user', async function() {
  const testUser = {'username':'tester',
   ...
   const { status, data, token } = await axios.post('/register',{'user':testUser},AXIOSCONFIG);
    assert.equal(status,200);
    ...
    const { secondStatus} = await axios.post('/register',{'user':testUser},AXIOSCONFIG);
...
When('saves a Session', async function() {
  ...
  const session =  { 'correct': 5,
                      ...
                      'card_sets_added': 5,
                      'user_id': 1};
  const { status } = await axios.post('/saveSession',{'session':session,'user':user},AXIOSCONFIG);
  assert.equal(status,200);
});
...
When('renames a Category', async function() {
  ...
  assert.equal(status,200);
  assert.ok(data.card_id);
  const { renameStatus } = await axios.post('/renameCategory',{'user':user,'newCat':'Birchums war stories','oldCat':'RENAMETESTCATEGORY'},AXIOSCONFIG);
...
When('adds a Category to a Collection', async function() {
  ...
  assert.equal(addCatToCollResult.cardsUpdated,10);
  const cardIdsToDelete = [];
  insertCardListResult.data.cards.forEach( (c) => { cardIdsToDelete.push(c.card_id); });
  const deleteStatus = await axios.post('/deleteCards',{'cardids':cardIdsToDelete},AXIOSCONFIG);
...
When('I delete all test data', async function() {
    // Delete inserted card:
    const db = new DAO().getDatabase();
    logger(`TEST DATA - card: ${this.testCardId} - session: ${this.testSessionId} - user: ${this.testUserId} - cardSet: ${this.testSetId}`);
