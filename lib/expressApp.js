...
import { authenticate } from './serverMiddleware';
...
// With Authenticate implemented all requests from these paths will
// have to also include the token and user id. 
router.post('/insertCard', authenticate, async (req, res) => {
...
router.post('/insertCardList', authenticate, async (req, res) => { 
...
router.post('/saveSession', authenticate, async (req, res) => {
...
router.post('/register', async (req, res) => {
  ...
  if( usernameExists.length === 0 ){
    ...
    let hashPass = hashSync(pw,salt);
    //Insert User, get their ID
    let registeredUser = await dataDAO.runQuery(queries.createUser,[un,hashPass,em,'Recruit']);
    ...
    */
    let token = uuidv4();
    global.authTokens.push({'userid':user.user_id,'token':token});
    user.token = token;
...
router.post('/deletUser', authenticate, async (req, res) => { 
...
router.post('/login', async (req, res) => {
  ...
  if(userQueryResponse.length === 1){
    ...
    if(compareSync(pw,user.password)){
      ...
      cardsResponse.forEach( (card) => { if(Object.values(card).includes(undefined)) {fail(`undefined value for: ${JSON.stringify(card)}`); }});
...
router.post('/createSet', authenticate, async (req,res) => {
...
router.post('/deleteCards', authenticate, async (req,res) => {
  const card_ids = req.body.cardids;
  const password = req.body.password;
  logger(`...      Deleting cards: ${card_ids}`);
  if(password !== 'monkeymeat') { res.sendStatus(502); return; }
  const delTestCard = await dataDAO.runQuery(`DELETE FROM FlashCards WHERE card_id IN (?)`,[card_ids]);
  if(delTestCard.affectedRows === card_ids.length) { 
    res.sendStatus(200); 
  } else { 
    res.sendStatus(500); fail(`card: ${card_ids} not deleted.`);  
  }
});

router.post('/renameCategory', authenticate, async (req,res) => {
  const newCat = req.body.newCat;
  const oldCat = req.body.oldCat;
  
  logger(`...       Renaming ${oldCat} to ${newCat}`);
  const renameResult = await dataDAO.runQuery(queries.renameCategory,[newCat,oldCat]);
  logger(`rename result ${JSON.stringify(renameResult)}`);
  res.send(200);
});
app.use('/', router);
