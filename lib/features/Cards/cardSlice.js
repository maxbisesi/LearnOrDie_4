...
const cardSlice = createSlice({
    ...
    reducers: {
            ...
            if(result === 'nailed' || result === 'whiffed') {
                ...
                switch(state.sequencingMode) {
                    ...
                    case `filter`: 
                        ...
                        let randomArray = state.filterCardIDs.slice(0);
                        randomArray = shuffle(randomArray);
                        state.filterCardIDs = randomArray;
        ...
        update (state, action) { 
            const { card_id, card, answer, category } = action.payload;
            console.log(`update card: ${card_id}, ${card}, ${answer}, ${category}`);
            const ind = state.cards.findIndex(c => c.card_id === card_id);

            state.cards[ind].card = card; 
            state.cards[ind].answer = answer;
            state.cards[ind].category = category;
            
            console.log(`new card in store: ${JSON.stringify(state.cards[ind])}`);
        },
        ...
        sortCardsById(state, action) {
            state.cards.sort((a,b) => {
                return a.card_id - b.card_id;
            });
        },
        deleteGuestCards(state, action) {
            state.cards.forEach( (guestCard) => {
                if(action.payload.includes(guestCard.category)) {
                    guestCard['DELETED'] = true;
                }
            });
        }
    }
...
export function insertCard(card) {
 ...
 export function asyncUpdateCard(card) {
    return async (dispatch, getState) => {
        ...
        if(updateResponse.status === 200) {
            ...
            console.log(`cardSlice updated card server response: ${JSON.stringify(updatedCard)}`);
 ...
 export function createCardSet(setname,desc,ids) {
    return async (dispatch, getState) => { 
        const user = getState().userSlice.user;
        console.log(`card slice createCardSet(): ${JSON.stringify(user)}`);
 ...
 export function asyncDeleteCards(cards) {
    return async (dispatch, getState) => {
        ...
        const user_id = user.user_id;
        // double check user is logged in
        if ( getState().userSlice.loggedIn === false && user_id === 'GUESTID' ) {
            fail('Guest in user Delet method.');
        }
...
export const { next, addCard, fillCategories, update, saveForReview, review, setCards, fillSets, pushCards, filter, addSet, studySet, deleteCards, renameCategory, newCollection, addCategoryToCollection, fillCollections, filterByCollections, removeCategoryFromCollection, unStudySet, deleteCardSet, sortCardsById, deleteGuestCards} = cardSlice.actions;
