...
router.use( (req, res, next) => {
  console.log(chalk.blue(req.method, req.url, req.path));
  console.log(chalk.bgYellow(req.originalUrl));
  ...
  if(knownToken === undefined) {
    ...
    res.status(501).send('undefined auth token');
      ...
      logger(`submitted Token: ${JSON.stringify(token)}`);
...
router.post('/saveSession', async (req, res) => {
    ...
    if( sessionResult.affectedRows === 1) {
      ...
      res.status(100).type('application/json').send({'sessionId':null,'message':'session not saved.'});
    }
...
//   const cardsResponse = await dataDAO.runQuery(queries.getUsersCards,[id]);
//   logger(`${JSON.stringify(cardsResponse)}`);
//   res.status(200).send({'cardCount':'3000'});
// });

router.post('/getCardCount', async (req,res) => {
  ...
  const cardsResponse = await dataDAO.runQuery(queries.getUsersCards,[id]);
  logger(`user_id: ${id} has ${cardsResponse.length} cards.`);
  res.status(200).send({'cardCount':cardsResponse.length});
});
