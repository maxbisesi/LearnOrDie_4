...
router.post('/insertCard', async (req, res) => {
  ...
  const newCard = dataDAO.runQuery(queries.insertCard,[card,answer,category,user_id]);
  const newCard_Id = newCard.insertId;
  const newCardRows = newCard.affectedRows;

  const linkRecord = dataDAO.runQuery(queries.insertFlashCardUser,[newCard_Id,user_id,0,0,user_id]);
...
router.post('/saveSession', async (req, res) => {
  // This is just for saving a users state when they leave or logout.
    ...
    console.log(chalk.blueBright('SERVER SAVE SESSION BEFORE UNLOAD'));
    ...
    card_sets_added,
    date,user_id */
    const sessionResult = dataDAO.runQuery(config.insertSession,[cards_seen, correct, incorrect, cards_added, points_added,  card_sets_added, user_id]);
...
router.post('/login', async (req, res) => {
  ...
  console.log(chalk.bgYellowBright(`username: ${un} passowrd:${pw}`));
  const userQueryResponse = dataDAO.runQuery(queries.getUser,[un]);
  console.log(chalk.bgYellowBright(`\nresponse: ${JSON.stringify(userQueryResponse)}`));
  if(userQueryResponse.length === 1){
    ...
    fail('ERROR: More than one user returned on Login.'); 
  }
});
