...
import { createCategoryId, validateMatch } from '../../Utils';
    ...
    async validateCategories(categoryTable) {
        for (const [category, count] of categoryTable) {
            ...
            if(this.categories.get(category).count === count) {
                logger(`    Actual   Expected`);
                logger(`    Pass: ${this.categories.get(category).count} = ${count}`);
                return true;
            } else {
                logger(`    Actual   Expected`);
                logger(`    Failed: ${this.categories.get(category).count} != ${count}`);
                return false;
            }
        }
