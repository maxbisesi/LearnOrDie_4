    ...
    return (
        <div id="test" style={{position: 'relative'}} onClickCapture={allClicks}>
           {/*<Test_QandA handleChange={handleChange} showAnswer={showAnswer} card={currentCard} answer={currentAnswer} category={currentCategory} />*/}
            <div id="areas">
                <textarea title="localCard" value={localCard} onChange={e => setLocalCard(e.target.value)} rows="20" cols="50" name="card"></textarea>
                <textarea title="localAnswer" value={localAnswer} onChange={e => setLocalAnswer(e.target.value)} rows="20" cols="40" name="answer"></textarea>
                <input title="localCategory" value={localCategory} onChange={e => setLocalCategory(e.target.value)} type="text" name="category" />
            </div>
            <div
            ...
            <div id="buttoncontrolgroup">
                <input type="button" title="show" name="show" onClick={toggleAnswer} value="Show"/>
                <input type="button" title="update id="update" value="Update" onClick={handleUpdate}/>
                <input type="button" title="nailed" name="nailed" onClick={nextCard} value="Nailed it"/>
                <input type="button" title="whiffed" name="whiffed" onClick={nextCard} value="Missed it"/>
                <input type="button" title="previous" name="whiffed" onClick={nextCard} value="Previous"/>
                <input type="button" title="comeback" name="comeback" onClick={comeBack} value="Come back to this one!"/>
                <input type="button" title="review" name="review" onClick={review} value="Review" />
                <p>
