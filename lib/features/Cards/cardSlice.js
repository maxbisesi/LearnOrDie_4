...
export function insertCard(card) {
 ...
 export function addCollection(name) {
    return async (dispatch, getState) => {
        dispatch(newCollection(name));
    }; 
 }

export const { next, addCard, fillCategories, update, saveForReview, review, setCards, fillSets, pushCards, filter, addSet, studySet, deleteCards, renameCategory, newCollection} = cardSlice.actions;
