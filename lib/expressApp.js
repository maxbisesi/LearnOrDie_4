...
router.post('/insertCardList', async (req, res) => { 
  ...
  */
  cardList.forEach((c) => {
      if(c.card === undefined || c.answer = undefined || c.category === undefined){ fail('/insertCardList: undefined attribute.')}
      vals.push([c.card,c.answer,c.category,owner_id]);
  });
  // Forgot, you can't get all the inserted ids !
  const newCard = await dataDAO.runQuery(queries.insertCard,[card,answer,category,user_id]);
...
router.post('/saveSession', async (req, res) => {
    ...
    const sessionResult = await dataDAO.runQuery(queries.insertSession,[cards_seen, correct, incorrect, cards_added, points_added, card_sets_added, user_id]);
...
router.post('/login', async (req, res) => {
  ...
  if(userQueryResponse.length === 1){
    ...
    if(user.password === pw) {
      ...
      logger(`card: ${JSON.stringify(cardsResponse[0])}`);
