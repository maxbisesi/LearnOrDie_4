...
const cardSlice = createSlice({
    ...
    reducers: {
        ...
        unStudySet(state,action) {
            ...
            // Remove this id from list of studied sets and removed card ids
            // If that was the last one then switch the sequence
            const id = action.payload;
            if(state.studySetIds.includes(id)) {
                const unstudyInd = state.studySetIds.indexOf(id);
                if(unstudyInd !== -1) {
                    const removedSet = state.studySetIds.splice(unstudyInd,1);
                    console.log(`Unstudied setid: ${removedSet}`);
                } else {
                    fail(`Trying to unstudy set_id:${id} indexof gave -1.`);
                }
            } else {
                fail(`Trying to unstudy set_id:${id} but its not in studySetId.`);
            }
        },
...
export const { next, addCard, fillCategories, update, saveForReview, review, setCards, fillSets, pushCards, filter, addSet, studySet, deleteCards, renameCategory, newCollection, addCategoryToCollection, fillCollections, filterByCollections, removeCategoryFromCollection, unStudySet} = cardSlice.actions;
