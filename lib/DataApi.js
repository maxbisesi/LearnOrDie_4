...
class DataApi {
  ...
  async addCards(cards) {
    ...
    */
    if(!Array.isArray(cards)) { throw new Error(`DataApi.addCards takes an array of cards not a ${typeof cards}`); }
     ...
     let insrt = `
      ...
      const db = this.getDatabase();
      try {
        const insertedCards = await db.query(multInsrt,[cards.map(card => [card.answer,card.card,card.category,card.times_wrong,card.times_right,card.fk_user_id])]);
