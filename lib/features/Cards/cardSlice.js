...
export function insertCard(card) {
 ...
 export function saveGuestCards() { 
    return async (dispatch, getState) => { 
        const cards = getState().cardSlice.cards;
        const user_id = getState().userSlice.user.user_id;
        if(cards.length > 0 && user_id !== 'GUESTID'){
            const response = await axios.post('/insertCardList',{'cards':cards, 'user_id':user_id});
            // Update cards in store now
        }else {
            console.log('cardSlice: saveGuestCards -> user hasnt created any');
        }
    };
 }
