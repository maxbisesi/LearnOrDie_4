    ...
    async validateField(field,value) {
        ...
        switch(field) {
            ...
            case `Edit Card: Question`: 
                ...
                const actual = await ecq.getAttribute('textContent');
                if(actual == value) {
                    logger(`Edit Card: Question = ${actual} matches value: ${value}`);
                    return true;
                } else {
                    logger(`Edit Card: Question = ${actual} does not match value: ${value}`);
                    return false;
                }
            case `Edit Card: Answer`:
                const eca = await this.driver.wait(until.elementLocated(By.id('editcardanswer')));
                const actualAns = await eca.getAttribute('textContent');
                if(actualAns == value) {
                    logger(`Edit Card: Answer = ${actualAns} matches value: ${value}`);
                    return true;
                } else {
                    logger(`Edit Card: Answer = ${actualAns} does not match value: ${value}`);
                    return false;
                }
            case `Edit Card: Category`:
                const ecc = await this.driver.wait(until.elementLocated(By.id('localCategory')));
                const actualCat = await ecc.getAttribute('textContent');
                if(actualCat == value) {
                    logger(`Edit Card: Category = ${actualCat} matches value: ${value}`);
                    return true;
                } else {
                    logger(`Edit Card: Category = ${actualCat} does not match value: ${value}`);
                    return false;
                }
            case `Edit Card: card_id indicator`: break;
