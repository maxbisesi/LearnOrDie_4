...
router.post('/insertCard', async (req, res) => {
  ...
  const newCard = dataDAO.runQuery(queries.insertCard,[card,answer,category]);
  ...
  // THIBK ABOUT THIS WHERE SHOULD OWNER ID BE HELD ???? >>>
  const dbCard = {'card_id':newCard_id, 'card':card, 'answer':answer, 'category':category, 'own'};
...
router.post('/updateCards', async (req,res) => {
  ...
  for (const item of cards) {
    const updCard = { 'card_id':item.cardid, 'card':item.card, 'answer':item.answer, 'category':item.cateogry };
