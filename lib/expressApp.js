...
router.use( (req, res, next) => {
  ...
  global.authTokens.forEach(toke => console.log(`${toke.user_id} = ${toke.token}`));
  console.log(`knownTOKEN: ${knownToken} -- global.authTokens: ${global.authTokens.length}`);
...
router.post('/login', async (req, res) => {
  ...
  if(userQueryResponse.length === 1){
    ...
    if(compareSync(pw,user.password)){
      ...
      // make token
      let token = uuidv4();
      global.authTokens.push({'user_id':user.user_id,'token':token});
      user.token = token;
