...
describe('Test the api calls', () => { 
    ...
    test('Insert a card list.', async (done) => {
        ...
        for(let i =0; i<1000; i++) {
            noIdCards.push({'card':`TESTCARD-${i}`,'answer':`TESTANSWER-${i}`,'category':`TESTCATEGORY-${i}`});
        }
    ...
    test.only('Test DB transactions', async (done) => {
        /*const db = new DAO().getDatabase();
        const trans = async (err) => {
            if (err) { throw err; }
            const res = await db.query(`INSERT INTO FlashCards(card,answer,category,owner_id) 
                                    VALUES(?,?,?,?)`,['TRANS Test card','trans test Answer','trans test Category',999]);
            const delres = await db.query(`DELETE FROM FlashCards WHERE owner_id=?`,[999]);
            db.commit((err) =>{ if (err) { throw err; } });
        };
        db.beginTransaction(trans);*/
        const _config = {
            connectionLimit: 5,
            host: "localhost",
            user: "root",
