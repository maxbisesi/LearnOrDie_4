...
export function checkRankUp(currentRank, currentPoints, badges ) {
  ...
  export function validateMatch(actual,expected) {
    // replace the &nbsp; chars with regular spaces
    actual = actual.replace(/\u00A0/gu, ' ');
    // remove any spaces that are just before a new line character so that
    // the feature file strings work correctly
    actual = actual.replace(/\s+(?=\n)/g, '');
    actual = actual.trim();
    logger(`  actual: ${actual}   expected:${expected}`);
    if(actual === expected) {
      logger(`Result: Pass`);
      return true;
    } else {
