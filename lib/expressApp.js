...
router.use( (req, res, next) => {
  // Authenticate
  if(req.path === '/' || req.path === '/login' || req.path === '/favicon.ico') { 
    next(); 
    return;
  }
  ...
  if(knownToken === undefined) {
    fail('undefined knownToken');
    res.sendStatus(501);
  } else if(knownToken.token === token) {
      console.log(chalk.greenBright(`Tokens match.`));
      return next();
  } else {
      console.log(chalk.redBright('403 tokens dont match'));
      res.sendStatus(403);
      return next();
  }
});
...
router.post('/insertCardList', async (req, res) => { 
  ...
  cardList.forEach( (c) => {
      ...
      vals.push([c.card,c.answer,c.category,owner_id,c.collection]);
  });
  ...
  if(newIDs.length === vals.length) {
    newIDs.forEach( (elm) => { 
      ...
      const c = { 'card_id':id,'card':card,'answer':ans,'category':cat,'owner_id':ownerid,'collection':collection};
      newCards.push(c);
    });
  }
  ...
  if(newCards.length === cardList.length) {
    ...
    res.type('application/json').send({'cards':newCards});
...
router.post('/register', async (req, res) => {
  ...
  if( usernameExists.length === 0 ){
    ...
    */
    let token = uuidv4();
    authTokens.push({'userid':user.user_id,'token':token});
    user.token = token;
...
router.post('/addCategoryToCollection', async (req,res) => {
    ...
    const result = await dataDAO.runQuery(queries.addCategoryToCollection,[collection,category]);
    logger(` add category to Collection result: ${JSON.stringify(result)}`);
