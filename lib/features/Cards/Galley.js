...
export default function Galley(props) {
    ...
    for (let j =0; j < cardSets.length; j++) {
        ...
        existingSets.push(<Set key={cardSets[j].set_id.toString()} set_id={cardSets[j].set_id} size={s} setname={n} description={d}/>);
    }
    ...
    // If Displaying Save Set Modal, animate in.
    // If your effect returns a function, React will run it when it is time to clean up
    useEffect(()=>{   
        if(showSetModal === true) {
