export const getCategoriesFromCards = cards => {
  ...
  cards.forEach((part, i, theArray) => {
    if (!categories.includes(theArray[i].category)) {
      categories.push({
        catid: 0,
        name: theArray[i].category,
        count: 0,
        rating: 0
      });
    } else {
      let category = categories.find(cat => cat.name === theArray[i].category);
      category.count++;
    }
  });
