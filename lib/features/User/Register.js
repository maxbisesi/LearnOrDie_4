    ...
    try {
      if (validateLogin() === true) {
         ...
         *  'user': username,
            ...
            'style':charStyle });
         */
        const response = await axios.post('/register', { 'username': username,
                                                         'password': password, 
                                                         'email': email, });
        if (response.status === 200) {
          ...
          fail('Unknown server response from /register.');
        }
      } 
    } catch (e) {
      ...
      <div className="registration">
        ...
        >
          <p>
            ...
            <input id="registerusername" type="text" value={username || ''} onChange={e => setUsername(e.target.value)} />
          </p>
          <p>
          ...
          <p>
            ...
            <input id="registerconfirm" type="password" value={confirm || ''} onChange={e => setConfirm(e.target.value)} />
          </p>
          <p>
            <span id="loginfield">email:</span>{' '}
            <input id="registeremail" type="text" name="email"  value={email || ''} onChange={e => setEmail(e.target.value)} />
          </p>
          <p>
            <input id="chooseyourdestiny" type="submit" name="register" value="Choose Your Destiny" onClick={handleSubmit} />
          </p>
        </form>
        {/**NEEDS WORK TODO */}
        {/*
