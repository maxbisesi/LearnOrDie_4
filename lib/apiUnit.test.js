...
describe('Test the api calls', () => { 
    ...
    afterAll( async(done) => {
        ...
        // RESET AUTO IDs after tests
        const getCardMaxID = await db.query('SELECT MAX(card_id) as maxid FROM FlashCards');
        logger(`max id after deletes: ${getCardMaxID[0].maxid}`);
        const nextAutoID = getCardMaxID[0].maxid;
        const resetAuto = await db.query(`ALTER TABLE FlashCards AUTO_INCREMENT = ?`,[nextAutoID]);
