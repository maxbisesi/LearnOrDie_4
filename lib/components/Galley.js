...
class Galley extends React.Component {
  constructor(props) {
    ...
    for (let i = 0; i < pageone; i++) {
      ...
      this.pageOneCards.push(
        <GalleyQuestion addToClicked={this.addToClicked} key={card.cardid} cardid={card.cardid} card={card.card} category={card.category} times_right={card.times_right} times_wrong={card.times_wrong} />
      );
    }
  ...
  addToClicked(id) { 
    if(this.state.clickedQuestions.includes(id)) { console.log('Question removed'); this.setState({ clickedQuestions: this.state.clickedQuestions.filter(cardid => cardid !== id) }); }
    else { console.log('Question added'); this.setState({ clickedQuestions: [...this.state.clickedQuestions, id] }); }
  }

  addToSet(e) {
    //this.state.clickedQuestions.forEach(el => console.log(el));
      ...
      <div className="galley">
        ...
        {/* Card Set creation */}
          ...
          <div className="galley-buttons-navigation">
            ...
            <input className="galley-button" type="button" value="-->" />
            <input className="galley-button" type="button" value="Add To Set" onClick={this.addToSet}/>
            <input className="galley-button" type="button" value="Delete" onClick={this.deleteCards} />
