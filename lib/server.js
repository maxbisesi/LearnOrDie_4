...
router.post('/login', async (req, res) => {
  ...
  console.log(chalk.bgYellowBright(`\nresponse: ${JSON.stringify(userQueryResponse[0])}`));
  if(userQueryResponse.length === 1){
    ...
    if(user.password === pw) {
      ...
      console.log(chalk.bgWhiteBright(`cards: ${cardsResponse.length}, avatar: ${avatar.length}, sets: ${setsQueryResult.length}`));
