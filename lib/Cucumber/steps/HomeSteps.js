...
Then('The {string} Collection has the following Categories:', {timeout:30000}, async function(collection, cattable) {
    logger(`The ${collection} Collection has the following Categories:`);
    await loginData.currentPage.getCollections();
    let cats = cattable['rawtable'];
    let result = await loginData.currentPage.validateCategoriesInCollection(collection,cats);
    if (result !== true) { throw new Error(`The Categories in ${collection} did not match: ${cattable}`); }
