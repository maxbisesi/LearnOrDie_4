...
import { tempId, binarySearch } from '../../Utils';
/*{
  name: "todos",
...
const cardSlice = createSlice({
    ...
    reducers: {
        ...
        studySet(state,action) {
            ...
            state.studySet = true;
            state.filterOn = false;
            if( !state.studySetIds.includes(id)) { state.studySetIds.push(action.payload); }
            // Get card IDs from set
            const setInd = binarySearch(state.cardSets,0,state.cardSets.length,id);
            state.cardSets[setInd].cards.forEach((c) => {
...
// returns the array index of state.cards
// where the searched card_id( i ) is located 
function binarySearch(arr,i,start,end) {
    // Make sure array is sorted by ID
    // Cards are sorted on login.
    //get the mid point
    //let m = Math.floor(arr.length / 2);
    let m = Math.floor(end - start);
    console.log(`midpoint: ${m}`);
    if(arr[m].card_id === i) {
        console.log(`match: ${i}`);
        return m;
    } else if(arr[m].card_id > i) {
        // lower half 
        // arr[m].card_id = 45, i = 33
        console.log(`lower half: ${m}`);
        return binarySearch(arr,i,0,m);
    } else if(arr[m].card_id < i) {
        //upper half
        console.log(`upper half: ${m}`);
        return binarySearch(arr,i,m,arr.length);
    } 
}
...
export const { next, addCard, fillCategories, update, saveForReview, review, setCards, fillSets, pushCards, filter, addSet, studySet} = cardSlice.actions;
