...
const userSlice = createSlice({
  ...
  initialState: {
    ...
    session: { session_id:'',cards_seen:0, correct:0, incorrect:0, cards_added:0, points_added:0, card_sets_added:0, 'user_id':0}
  },
  reducers: {
    login(state, action) {
      //state.user = payload;
      ...
      let _userrank = checkRankUp(action.payload.userrank,action.payload.points,0);
      state.user.user_id = action.payload.user_id;
      state.user.username = action.payload.username;
      state.user.email = action.payload.email;
      state.user.points = action.payload.points;
      state.user.userrank = _userrank;
      state.loggedIn = true;
    },
