...
export default function Chum() {
  ...
  function submitCard() {
    ...
    if(state.image!== 'none'&&availableShapes.includes(state.image)) {
      console.log(`inserting image: ${state.image}`);
  ...
  let initialState = {'card':'','answer':'','category':'','focusedInput':'','image':null};

  function addShape(shapeName) {
    let image;
    switch(shapeName) {
      case 'Equilateral': image = (<img className="chum-questionImage-equilateral" src={equilateral} />); break;
      default: throw new Error('Chum: Unknown image type.');
    }
    localDispatch({'type':'addShape','value':image});
  }
  ...
  let chumQuestionClass = classNames({
    "chum-question": true,
    "chum-question-image": state.image !== null
  });

  let chumImageClass = classNames({
    "chum-questionImage-container": state.image !== null,
    "chum-noImage": state.image === null
  });
